...
public:
  ...
  unsigned int get_freq() const { return root->freq; }

  unsigned char get_char() const { return root->ch; }

  void set_freq(unsigned int f) { root->freq = f; }

  void set_char(unsigned char c) { root->ch = c; }
   ...
   */
  Node *get_left() const { return root->left; }

  /**
   * Getter. Getting right of tree.
   */
  Node *get_right() const { return root->right; }
   ...
   */
  void set_left(Node *N) { root->left = N; }
   ...
   * @param N another tree
   */
  void set_right(Node *N) { root->right = N; }
   ...
   */
  bool operator==(const Tree &T) const { return (root->freq == T.root->freq); }
